  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        country: { type: "string", description: "国コード（例: JP, US）" },
        venue_id: { type: "string", description: "会場ID（UUID）" },
        venue: { type: "string", description: "会場名（英語）" },
        venue_ja: { type: "string", description: "会場名（日本語）" },
        venue_address: { type: "string", description: "会場住所（英語）" },
        venue_address_ja: { type: "string", description: "会場住所（日本語）" },
        venue_access: { type: "string", description: "会場アクセス（英語）" },
        venue_access_ja: { type: "string", description: "会場アクセス（日本語）" },
        venue_image_url: { type: "string", description: "会場画像URL" },
        ...
        registration_url: { type: "string", description: "申込URL" },
        registration_deadline: { type: "string", description: "申込締切日（YYYY-MM-DD）" },
        entry_fee: { type: "string", description: "参加費（英語）" },
        entry_fee_ja: { type: "string", description: "参加費（日本語）" },
        contact_email: { type: "string", description: "連絡先メール" },
        contact_url: { type: "string", description: "連絡先URL" },
        rules: { type: "string", description: "ルール（英語）" },
        rules_ja: { type: "string", description: "ルール（日本語）" },
        weight_classes: { type: "array", description: "階級リスト", items: { type: "string" } },
        is_international: { type: "boolean", description: "国際大会フラグ" },
        notes: { type: "string", description: "備考（英語）" },
        notes_ja: { type: "string", description: "備考（日本語）" },
        related_tournament_slug: { type: "string", description: "関連大会スラッグ" },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        spectator_capacity: { type: "number", description: "観客席数" },
        mat_area_sqm: { type: "number", description: "マット面積（㎡）" },
        access_info: { type: "string", description: "アクセス情報（英語）" },
        access_info_ja: { type: "string", description: "アクセス情報（日本語）" },
        nearest_station: { type: "string", description: "最寄駅（英語）" },
        nearest_station_ja: { type: "string", description: "最寄駅（日本語）" },
        parking_info: { type: "string", description: "駐車場情報（英語）" },
        parking_info_ja: { type: "string", description: "駐車場情報（日本語）" },
        phone: { type: "string", description: "電話番号" },
        email: { type: "string", description: "メールアドレス" },
        website: { type: "string", description: "ウェブサイトURL" },
        google_maps_url: { type: "string", description: "GoogleマップURL" },
        image_url: { type: "string", description: "画像URL" },
        facilities: { type: "array", description: "設備リスト", items: { type: "string" } },
        rental_cost: { type: "string", description: "レンタル費用（英語）" },
        rental_cost_ja: { type: "string", description: "レンタル費用（日本語）" },
...
async function executeTool(toolName: string, args: Record<string, unknown>, permissions: string[]): Promise<unknown> {
  ...
  if (toolName === 'update_tournament') {
    ...
    // Map alternative field names to DB column names
    if (rest.start_date && !rest.date_start) {
      updateData.date_start = rest.start_date;
      delete updateData.start_date;
    }
    if (rest.end_date && !rest.date_end) {
      updateData.date_end = rest.end_date;
      delete updateData.end_date;
    }
    console.log('[MCP Server] Updating tournament with data:', JSON.stringify(updateData));
    const { data, error } = await supabase.from('tournaments').update(updateData).eq('id', id).select().single();
    if (error) {
      console.error('[MCP Server] Tournament update error:', JSON.stringify(error));
      throw error;
    }
    return data;
  }
