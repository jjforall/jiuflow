...
### 🔍 SEO対策
- **OGP画像**: 各ページに動的OGP画像を設定（大会、選手、道場、動画等）
- **サイトマップ**: `public/sitemap.xml`で検索エンジン向けサイトマップを提供
- **メタタグ**: 各ページに適切なtitle、description、canonical URLを設定
- **構造化データ**: セマンティックHTMLによる適切なマークアップ

### 📊 アナリティクス（GDPR対応）
- **Google Analytics 4**: ページビュー、コンバージョン、ユーザー行動を追跡
- **Consent Mode v2**: Cookie同意に基づいた適切なデータ収集
- **コンバージョントラッキング**: 
  - サインアップ（メール・Google）
  - サブスクリプション購入
  - 動画視聴エンゲージメント
- **E-commerce追跡**: サブスクリプション購入の詳細追跡
- **プライバシー保護**: 
  - デフォルトで全Cookie拒否（GDPRコンプライアンス）
  - ユーザー同意後のみトラッキング開始
  - IP匿名化
