...
import { SEOHead, generateAthleteStructuredData, generateBreadcrumbStructuredData } from "@/components/SEOHead";
...
const Athlete = () => {
  ...
  // SEO data is now handled by SEOHead component
  ...
  const pageUrl = `/athletes/${celebrity.slug || slugOrUsername}`;
  const athleteDescription = celebrity.bio 
    ? celebrity.bio.slice(0, 160) 
    : language === "ja" 
      ? `${celebrity.display_name}の選手プロフィール - JiuFlow` 
      : language === "pt"
        ? `Perfil do atleta ${celebrity.display_name} - JiuFlow`
        : `${celebrity.display_name} athlete profile - JiuFlow`;

  const structuredData = generateAthleteStructuredData({
    display_name: celebrity.display_name,
    bio: celebrity.bio,
  ...
  const breadcrumbData = generateBreadcrumbStructuredData([
    ...
    { name: language === 'ja' ? '選手一覧' : 'Athletes', url: '/athletes' },
    { name: celebrity.display_name, url: pageUrl },
  ]);

  return (
    <div className="min-h-screen bg-background">
      <SEOHead
        title={`${celebrity.display_name} | JiuFlow`}
        description={athleteDescription}
        ogImage={celebrity.avatar_url || undefined}
        ogType="profile"
        canonicalUrl={pageUrl}
        structuredData={{ "@graph": [structuredData, breadcrumbData] }}
        keywords={['柔術', 'BJJ', '選手', celebrity.display_name, celebrity.home_dojo || '']}
      />
      <Navigation />
